...
For example
C:\Users\<username>\Documents\Arduino\libraries\WiFi101\src\bus_wrapper\source\nm_bus_wrapper_samd21.cpp
...
#define WINC1501_SPI SPI1
#include "WiFi101.h"
//If you get Compilation error: WiFi101_SPI1.h: No such file or directory, then you'll have to make it.
...
void loop() {
  ...
  if (RXTimer0 > 500) BLUE_LED_state = HIGH;
...
void processCan0(const CAN_message_t &msg) {
  ...
  if (msg.id == 0x19f21139){ // Water level sensor on ballast pumps
      printCAN(msg,0);
      RED_LED_state = msg.buf[0]; //Center
      YELLOW_LED_state = msg.buf[1]; //Port
      GREEN_LED_state = msg.buf[2]; // Starboard
  }
}
